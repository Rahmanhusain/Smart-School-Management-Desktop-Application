...
public class AddUser implements Initializable {
    ...
    int search() throws SQLException {
        ArrayList<usermodel> user = this.c.Retrieveuser((String)this.access.getValue());
        if (!user.isEmpty()) {
    ...
    void add() throws SQLException {
        if (this.search() == -1) {
            this.c.Insertuser(this.password.getText(), this.userid.getText(), this.Email.getText(), (String)this.access.getValue());
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        this.access.getItems().addAll(this.userch);
        this.access.setValue("Admin");
    }
}
